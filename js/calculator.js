async function calcularPorcentaje() {
    ...
    const consumoTotalKwhMes = parseFloat(document.getElementById('consumoTotal').value);
    ...
    try {
        // Leer el archivo CSV
        const response = await fetch('../archive2/03_modern_renewable_prod.csv');
        ...
        const solarIndex = headers.indexOf('Electricity from solar (TWh)');
        
        if (entityIndex === -1 || yearIndex === -1 || solarIndex === -1) {
        ...
        // Obtener el valor de electricidad solar
        const electricityFromSolarTwh = parseFloat(row[solarIndex]);
        if (isNaN(electricityFromSolarTwh) || electricityFromSolarTwh <= 0) {
            throw new Error("El valor de electricidad solar es inválido.");
        }

        // Calcular el porcentaje de consumo del usuario
        const porcentajeUsuario = (valorUsuariosTwhAño / electricityFromSolarTwh) * 100;
        ...
        document.getElementById('resultado-porcentaje').innerText =
            `Tu consumo anual corresponde al ${resultado}% de la producción solar de Colombia en 2021.`;
